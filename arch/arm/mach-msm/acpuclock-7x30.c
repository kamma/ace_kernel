...
void acpuclk_set_vdd(unsigned int acpu_khz, int vdd)
{
    int i;
    pr_info("acpuclk_set_vdd\n");
    ...
    for (i = 0; acpu_freq_tbl[i].acpu_clk_khz; i++)
    {
	    ...
	    {
		if (acpu_khz == 0) {
		    pr_info("acpuclk_set_vdd-pre for %8u : mv=%4d raw=%4d\n", acpu_freq_tbl[i].acpu_clk_khz, acpu_freq_tbl[i].vdd_mv, acpu_freq_tbl[i].vdd_raw);
		    acpu_freq_tbl[i].vdd_mv = min(max((int)(acpu_freq_tbl[i].vdd_mv + vdd), (int)875), (int)1300);
		    acpu_freq_tbl[i].vdd_raw = VDD_RAW(min(max((int)(acpu_freq_tbl[i].vdd_mv + vdd), (int)875), (int)1300));
